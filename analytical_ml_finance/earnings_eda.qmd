...
.sidebar-item-text { color: var(--text-muted) !important; font-size: 12px !important; }
.sidebar-item-text:hover, .active .sidebar-item-text { color: var(--text-primary) !important; }
...
p, li { color: var(--text-muted) !important; font-size: 13px; line-height: 1.75; }
...
.callout-body p { color: var(--text-muted) !important; }
...
tbody td { color: var(--text-muted) !important; padding: 7px 12px !important; }
...
my_theme <- hc_theme(
  ...
  subtitle = list(style = list(color = "#8888aa", fontSize = "11px")),
  xAxis = list(
    gridLineColor = "#1e1e30", lineColor = "#2a2a42", tickColor = "#2a2a42",
    labels = list(style = list(color = "#8888aa", fontSize = "11px")),
    title  = list(style = list(color = "#8888aa", fontSize = "11px"))
  ),
  yAxis = list(
  ...
  legend = list(
    itemStyle      = list(color = "#8888aa", fontSize = "11px", fontWeight = "400"),
    itemHoverStyle = list(color = "#e8e8f0")
  ),
  tooltip = list(
...
# Top-500 subset — used for all heavy NLP
...
```{r}
earnings_calls_raw$fiscal_year <- earnings_calls_raw$fiscal_year %>% as.double()
```

```{python}

head(r.earnings_call_raw)
...
# Reticulate encodes R NA integers as -2147483648 — replace with NaN
df['fiscal_year'] = pd.to_numeric(df['fiscal_year'], errors='coerce') \
                      .replace(-2147483648, np.nan)

print("=" * 55)
print(f"  Dataset shape          : {df.shape[0]:>10,} rows × {df.shape[1]} cols")
print(f"  Unique tickers         : {df['ticker'].nunique():>10,}")
print(f"  PERMNO coverage        : {df['permno'].notna().mean():>10.1%}")

yr_min = int(df['fiscal_year'].dropna().min())
yr_max = int(df['fiscal_year'].dropna().max())
print(f"  Fiscal years           : {yr_min} – {yr_max}")
print("=" * 55)
...
::: {.callout-note}
**NLP scope note:** Sections 3.3 – 3.9 are restricted to the **top-50 S&P 500 firms by index weight** (`n = {r format(nrow(earnings_calls), big.mark=",")} transcripts`). Section 3.2 (Word Cloud) uses the full corpus.
:::
...
hw_grid(hc_year, hc_firm, ncol = 2)
```
...
hw_grid(charts_tfidf[[1]], charts_tfidf[[2]], charts_tfidf[[3]],
        charts_tfidf[[4]], charts_tfidf[[5]], charts_tfidf[[6]],
        charts_tfidf[[7]], charts_tfidf[[8]], charts_tfidf[[9]],
        ncol = 3)
```
...
hw_grid(charts_lda[[1]], charts_lda[[2]], charts_lda[[3]], charts_lda[[4]],
        charts_lda[[5]], charts_lda[[6]], charts_lda[[7]], charts_lda[[8]],
        ncol = 4)
```
...
hw_grid(hc_ret, hc_scatter, ncol = 2)
```
...
# Pull tokens already computed in R (top-50 corpus)
tok = r.tokens[['ticker', 'word', 'fiscal_year']].copy()

print(f"Total tokens (top-50): {len(tok):,}")
print(f"Unique terms           : {tok['word'].nunique():,}")

# Top 25 terms across corpus
top_terms = tok['word'].value_counts().head(25)
print("\nTop 25 terms by frequency:")
print(top_terms.to_string())

# Token count by firm
by_firm = tok.groupby('ticker').size().sort_values(ascending=False)
print("\nTokens by firm (top 10):")
print(by_firm.head(10).to_string())
```
...
import pandas as pd
sample_df = r.earnings_calls[['ticker', 'fiscal_year', 'full_transcript_text']].copy()
...
highchart() |>
  ...
  hc_annotations(list(
    labels = list(list(
      ...
      style = list(color = "#8888aa", fontSize = "13px")
    ))
  ))
```
...
sample = (
    r.earnings_calls[['ticker', 'fiscal_year', 'full_transcript_text']]
     .sample(min(500, len(r.earnings_calls)), random_state=42)
     .copy()
)
