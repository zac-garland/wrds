...
# ── Summary stats → R renders as kable ────────────────────────────────────────
yr_min = int(df['fiscal_year'].dropna().min())
yr_max = int(df['fiscal_year'].dropna().max())

summary_df = pd.DataFrame({
    'Metric': ['Dataset shape', 'Unique tickers', 'PERMNO coverage', 'Fiscal years'],
    'Value':  [
        f"{df.shape[0]:,} rows × {df.shape[1]} cols",
        str(df['ticker'].nunique()),
        f"{df['permno'].notna().mean():.1%}",
        f"{yr_min} – {yr_max}"
    ]
})

# ── Missing values table ───────────────────────────────────────────────────────
missing_df = df.isnull().sum().rename("Missing").reset_index()
missing_df.columns = ['Column', 'Missing']
missing_df['% Missing'] = (missing_df['Missing'] / len(df) * 100).round(1)

# ── Word count distribution ────────────────────────────────────────────────────
wc_desc = df['word_count'].describe().round(0).reset_index()
wc_desc.columns = ['Statistic', 'Word Count']
wc_desc['Word Count'] = wc_desc['Word Count'].astype(int)
```
...
# ── Summary ───────────────────────────────────────────────────────────────────
kable(reticulate::py_to_r(py$summary_df), align = c("l","l"), caption = "Dataset Summary") |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE, position = "left", font_size = 12)
```
...
# ── Missing values ─────────────────────────────────────────────────────────────
kable(reticulate::py_to_r(py$missing_df), align = c("l","r","r"), caption = "Missing Values per Column") |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE, position = "left", font_size = 12)
```
...
# ── Word count distribution ────────────────────────────────────────────────────
kable(reticulate::py_to_r(py$wc_desc), align = c("l","r"), caption = "Word Count Distribution") |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE, position = "left", font_size = 12)
```
...
# Top 25 terms → R renders as kable
top_terms_df = (
    tok['word'].value_counts()
    .head(25)
    .reset_index()
    .rename(columns={'word': 'Term', 'count': 'Frequency'})
)

# Tokens by firm → R renders as kable
by_firm_df = (
    tok.groupby('ticker').size()
    .sort_values(ascending=False)
    .head(10)
    .reset_index()
    .rename(columns={'ticker': 'Ticker', 0: 'Token Count'})
)

# Summary scalars for inline display
total_tokens  = len(tok)
unique_terms  = tok['word'].nunique()
```
...
tagList(
  tags$p(
    tags$strong(glue::glue("Total tokens (top-50): {format(py$total_tokens, big.mark=',')}")),
    tags$span(" · "),
    tags$strong(glue::glue("Unique terms: {format(py$unique_terms, big.mark=',')}"))
  ),
  tags$div(
    style = "display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-top:12px;",
    tags$div(
      kable(reticulate::py_to_r(py$top_terms_df), caption = "Top 25 Terms by Frequency") |>
        kable_styling(bootstrap_options = c("striped","hover","condensed"),
                      full_width = TRUE, font_size = 12) |>
        htmltools::HTML()
    ),
    tags$div(
      kable(reticulate::py_to_r(py$by_firm_df), caption = "Token Count by Firm (Top 10)") |>
        kable_styling(bootstrap_options = c("striped","hover","condensed"),
                      full_width = TRUE, font_size = 12) |>
        htmltools::HTML()
    )
